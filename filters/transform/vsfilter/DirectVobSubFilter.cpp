...
///////////////////////////////////////////////////////////////////////////////
STDMETHODIMP CDirectVobSubFilter::SetPGSAndLoadImmediately(BYTE* buf, int bufSize)
{
    CComPtr<ISubStream> pSubStream;
    CAutoPtr<EmbeddedPGSSubtitleProvider> pgs(DEBUG_NEW EmbeddedPGSSubtitleProvider());
    if (pgs && pgs->LoadBytes(buf, bufSize) && pgs->GetStreamCount() > 0)
    {
        pSubStream = pgs.Detach();
    }
...
void CDirectVobSubFilter::SetSubtitle(ISubStream* pSubStream, bool fApplyDefStyle)
{
	...
	if(pSubStream)
	{
        ...
        else if (clsid == __uuidof(EmbeddedPGSSubtitleProvider))
        {
            CompositionObject::ColorType color_type = CompositionObject::NONE;
            CompositionObject::YuvRangeType range_type = CompositionObject::RANGE_NONE;
            if (m_xy_str_opt[STRING_PGS_YUV_RANGE].CompareNoCase(_T("PC")) == 0)
            ...
            EmbeddedPGSSubtitleProvider* sub = dynamic_cast<EmbeddedPGSSubtitleProvider*>(pSubStream);
            sub->SetYuvType(color_type, range_type);
