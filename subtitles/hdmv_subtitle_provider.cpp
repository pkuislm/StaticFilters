...
bool SupFileSubtitleProvider::Open(CString fn, CString subName /*= _T("")*/)
{
    ...
    return CAMThread::Create()==TRUE;//This starts SupFileSubtitleProvider::ThreadProc()
}
...
DWORD SupFileSubtitleProvider::ThreadProc()
{
    ...
    int len;
    BYTE buff[65536];//this is probably the result of consideration for performance?
...
//
EmbeddedPGSSubtitleProvider::EmbeddedPGSSubtitleProvider()
    : CSubPicProviderImpl(&m_csCritSec)
{
    m_pSub = nullptr;
    m_helper = nullptr;
    m_rtStart = 0;
...
EmbeddedPGSSubtitleProvider::~EmbeddedPGSSubtitleProvider()
{
    if (m_pSub != nullptr) delete m_pSub;
    if (m_helper != nullptr) delete m_helper;
}

STDMETHODIMP EmbeddedPGSSubtitleProvider::NonDelegatingQueryInterface(REFIID riid, void** ppv)
{
    CheckPointer(ppv, E_POINTER);
    *ppv = NULL;
...
bool EmbeddedPGSSubtitleProvider::LoadBytes(BYTE* buf, int bufSize)
{
    ...
    CMemFile sub;
    sub.SetLength(bufSize);
    sub.SeekToBegin();
    sub.Write(buf, bufSize);
    sub.SeekToBegin();
...
STDMETHODIMP EmbeddedPGSSubtitleProvider::GetClassID(CLSID* pClassID)
{
    return pClassID ? *pClassID = __uuidof(this), S_OK : E_POINTER;
}
...
STDMETHODIMP_(POSITION) EmbeddedPGSSubtitleProvider::GetStartPosition(REFERENCE_TIME rt, double fps)
{
    CAutoLock cAutoLock(&m_csCritSec);
    return	m_pSub->GetStartPosition(rt - m_rtStart, fps);
}

STDMETHODIMP_(POSITION) EmbeddedPGSSubtitleProvider::GetNext(POSITION pos)
{
    CAutoLock cAutoLock(&m_csCritSec);
    return m_pSub->GetNext(pos);
}

STDMETHODIMP_(REFERENCE_TIME) EmbeddedPGSSubtitleProvider::GetStart(POSITION pos, double fps)
{
    CAutoLock cAutoLock(&m_csCritSec);
    return m_pSub->GetStart(pos) + m_rtStart;
}

STDMETHODIMP_(REFERENCE_TIME) EmbeddedPGSSubtitleProvider::GetStop(POSITION pos, double fps)
{
    CAutoLock cAutoLock(&m_csCritSec);
    return m_pSub->GetStop(pos) + m_rtStart;
}

STDMETHODIMP_(bool) EmbeddedPGSSubtitleProvider::IsAnimated(POSITION pos)
{
    return false;
}

STDMETHODIMP EmbeddedPGSSubtitleProvider::Render(SubPicDesc& spd, REFERENCE_TIME rt, double fps, RECT& bbox)
{
    CAutoLock cAutoLock(&m_csCritSec);
...
STDMETHODIMP EmbeddedPGSSubtitleProvider::GetTextureSize(POSITION pos, SIZE& MaxTextureSize, SIZE& VideoSize, POINT& VideoTopLeft)
{
    CAutoLock cAutoLock(&m_csCritSec);
...
STDMETHODIMP_(bool) EmbeddedPGSSubtitleProvider::IsColorTypeSupported(int type)
{
...
STDMETHODIMP_(int) EmbeddedPGSSubtitleProvider::GetStreamCount()
{
    return 1;
}

STDMETHODIMP EmbeddedPGSSubtitleProvider::GetStreamInfo(int iStream, WCHAR** ppName, LCID* pLCID)
{
    if (iStream != 0) {
        return E_INVALIDARG;
    }
...
STDMETHODIMP_(int) EmbeddedPGSSubtitleProvider::GetStream()
{
    return 0;
}

STDMETHODIMP EmbeddedPGSSubtitleProvider::SetStream(int iStream)
{
    return iStream == 0 ? S_OK : E_FAIL;
}

STDMETHODIMP EmbeddedPGSSubtitleProvider::Reload()
{
    return S_OK;
}

HRESULT EmbeddedPGSSubtitleProvider::SetYuvType(CBaseSub::ColorType colorType, CBaseSub::YuvRangeType yuvRangeType)
{
    CAutoLock cAutoLock(&m_csCritSec);
    return m_pSub->SetYuvType(colorType, yuvRangeType);
}
